...
## Reusable placeholder component & car rating/review panel

**Requirement:** A single reusable "empty state" LWC, with both its
message *and* its icon/illustration configurable per context, used
wherever a query returns no results — plus a Ratings & Reviews panel
for a car showing an average-rating summary and a detailed,
per-review list (rating, comments, date), using that placeholder for
the no-reviews case.

**Placeholder component.** `c/placeholder` (`isExposed: false` —
correctly an internal building block, not something added directly via
App Builder) takes `@api message` and renders it next to the
`CarOnRentalLogo` image.

**Gap against the requirement:** only the message is configurable — the
icon/illustration is hardcoded to the company logo with no `@api`
property to override it. The requirement is explicit that *both* need
to be configurable per context, with "a car icon for an empty list of
cars" as its own example of a *different* icon than a generic logo.
Adding something like `@api iconUrl` (defaulting to the current logo
when not supplied) would close this.

**Star rating component.** `c/starRating` reuses the `fivestar`
Aura-bundled static resource — the same one installed earlier for the
Car Return Checklist flow — from LWC, since LWC can't consume an Aura
component directly. `loadScript`/`loadStyle` via
`lightning/platformResourceLoader`, gated by an `isRendered` flag
inside `renderedCallback` so the script/style pair loads exactly once
per component instance — precisely the pattern described going in, and
correctly implemented.

**`CarReviewController.getCarReview`** (`@AuraEnabled(cacheable=true)`)
returns the car's average rating, all its reviews, and a 1–5 star
distribution count, pre-seeding the distribution map so every star
value has an entry even at zero. One thing worth double-checking: it
builds the distribution by casting `Rating__c` straight to `Integer`
(`(Integer)review.Rating__c`), which *truncates* rather than rounds. If
the star widget only ever allows whole-number ratings this is moot, but
if it can produce half-stars (some builds of the fivestar plugin
support this), a 3.5 truncates to 3 instead of rounding to the nearer
bucket, and any rating that ends up outside the pre-seeded 1–5 keys
(including a null `Rating__c`, if that's ever possible) would throw a
null-pointer error on the map lookup rather than being handled.

**`c/carRatingReview`**, added to `Car_Record_Page`: shows the average
rating (via `c-star-rating`, read-only) with the total review count,
a per-star distribution bar chart with percentages, and the detailed
...
are unfilled CLI-generated stubs — `expect(1).toBe(1)`, the
placeholder's own comment reading "TODO: test case generated by CLI
command, please fill in test logic." None of them exercise anything,
including either bug above — the same "assertion that can't fail"
pattern flagged earlier on the Apex side, now on the LWC side too.

## Car image manager component

**Requirement:** A carousel showing all of a car's images (with the
placeholder for none), PNG/JPG upload that's automatically resized to
500×500 and compressed, and the ability to designate one image as
primary.

**Solution:** `c/carImageManager` (`carImageController` behind it) —
`lightning-carousel` over `getCarImages`'s wired data, gated by a
`hasProductImages` getter that correctly checks `.length > 0` this
time (unlike the truthy-empty-array bug documented and fixed earlier
for `carRatingReview` — this one was built right from the start),
falling through to `c-placeholder` when there are none.
`lightning-file-upload` restricted to `.png,.jpg,.jpeg`, with a
"Primary Image" checkbox alongside it; on upload, `createFile` looks up
the just-uploaded `ContentVersion` and inserts a `Car_Image__c`
pointing at it, then the component calls `refreshApex` and
`notifyRecordUpdateAvailable` so the carousel and any other component
...
`/sfc/servlet.shepherd/version/download/{ContentVersion.Id}` — a
relative, session-gated Salesforce Files URL that only resolves for a
user with an authenticated Salesforce session viewing it inside
Salesforce's own domain. The `Sync Primary Image` flow (documented a
few sessions back) copies exactly this value into
`Car__c.Primary_Image_Url__c` whenever an image is marked primary — and
`Primary_Image_Url__c` is precisely the field the customer-facing car
availability REST API (also documented earlier) returns as `imageUrl`
to external consumers. An external app calling that API would get an
image URL that doesn't actually work for it — not absolute, and gated
behind Salesforce auth it doesn't have. Fixing this needs either a
public/guest-accessible file distribution (e.g. `ContentDistribution`)
...
the new primary — and un-primaries whatever was primary before, via
the same `Sync Primary Image` flow — unless whoever's uploading
remembers to uncheck it each time. A car's cover photo could keep
changing unexpectedly as more pictures get added. Defaulting to
